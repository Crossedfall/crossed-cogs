...
class GetNotes(BaseCog):
    ...
    async def notes(self, ctx, *, ckey: str):
        ...
        query = f"SELECT timestamp, playtime, adminckey, text, type FROM {prefix}messages WHERE targetckey=%s AND deleted = 0 ORDER BY timestamp DESC"
        ...
        try:
            ...
            for row in rows:
                total += 1
                notes += f"\n[{row['timestamp']} | {row['type']} by {row['adminckey']} at {row['playtime'] // 60} hours]\n{row['text']}"
